...
bool VulkanRenderer::init(ANativeWindow* window) {
    ...
    // ==========================================
    // معالجة مشكلة الانعكاس عبر الـ Negative Viewport
    // ==========================================
    VkViewport viewport{
        0.0f,
        (float)swapchainExtent.height,      // البدء من الأسفل
        (float)swapchainExtent.width,
        -(float)swapchainExtent.height,     // ارتفاع سالب لعكس المحور للأعلى كبلندر
        0.0f,
        1.0f
    };
    ...
    // (1) بايبلاين المكعب بشيدر بلندر الاستوديو
    ...
    // (2) بايبلاين خطوط شبكة الأرضية ومحاور الجزمو
    ...
    // (3) بايبلاين مخاريط الجزمو الصلبة (3D Solid Cones) - تُرسم كمثلثات مصمتة فوق كل شيء
    ...
    gPdssi.depthTestEnable = VK_FALSE; // إظهار الجزمو دائماً فوق المكعب
    ...
    // تجهيز أرضية بلندر وشريط الأدوات
    ...
    // تجهيز الجزمو الصلب
    gizmo.init();
    gizmoVertexCount = (uint32_t)gizmo.coneVertices.size();
    createBuffer(gizmo.coneVertices.size() * sizeof(GizmoVertex), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, gizmoVbo, gizmoVboMemory);
    void* gizmoData;
    vkMapMemory(device, gizmoVboMemory, 0, gizmo.coneVertices.size() * sizeof(GizmoVertex), 0, &gizmoData);
    memcpy(gizmoData, gizmo.coneVertices.data(), gizmo.coneVertices.size() * sizeof(GizmoVertex));
    ...
    LOGI("نجح إقلاع محرك Vulkan بعد معالجة الاتجاهات وتثبيت الجزمو الصلب!");
    return true;
}
...
void VulkanRenderer::renderFrame() {
    ...
    // لون خلفية استوديو بلندر الرمادية الأصلية (#30333A)
    ...
    // 1. رسم شبكة أرضية بلندر تحت المكعب
    ...
    // 2. رسم المكعب المصمت بشيدر بلندر الاستوديو
    ...
    // 3. رسم مخاريط الجزمو ثلاثية الأبعاد الصلبة (Solid Cones)
