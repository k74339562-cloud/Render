...
// مؤشر دائم لذاكرة الـ UBO لمنع خنق المعالج وتسريع الإطارات
...
static VkShaderModule createShaderModule(VkDevice device, const uint32_t* code, size_t sizeBytes) {
    ...
    if (vkCreateShaderModule(device, &ci, nullptr, &mod) != VK_SUCCESS) {
        LOGE("فشل في إنشاء Shader Module!");
    }
    return mod;
}
...
bool VulkanRenderer::init(ANativeWindow* window) {
    // تنظيف أي موارد قديمة لمنع أي تسريب للذاكرة عند إعادة التدوير
    if (device != VK_NULL_HANDLE) {
        cleanup();
    }

    // 1. إنشاء VkInstance لنظام أندرويد
    const char* ext[] = {
    ...
    if (vkCreateInstance(&ici, nullptr, &instance) != VK_SUCCESS) {
        LOGE("فشل إنشاء VkInstance");
        return false;
    }

    // 2. ربط نافذة أندرويد بالسطح الرسومي
    VkAndroidSurfaceCreateInfoKHR sci{VK_STRUCTURE_TYPE_ANDROID_SURFACE_CREATE_INFO_KHR};
    sci.window = window;
    if (vkCreateAndroidSurfaceKHR(instance, &sci, nullptr, &surface) != VK_SUCCESS) {
        LOGE("فشل إنشاء Surface");
        return false;
    }

    // 3. فحص واختيار كرت الشاشة الأقوى في الهاتف
    uint32_t devCount = 0;
    ...
    // 4. إنشاء الـ Device وربط طابور الرسم
    float qp = 1.0f;
    ...
    if (vkCreateDevice(physicalDevice, &dci, nullptr, &device) != VK_SUCCESS) {
        LOGE("فشل إنشاء Logical Device");
        return false;
    }
    vkGetDeviceQueue(device, 0, 0, &graphicsQueue);

    // 5. إنشاء الـ Swapchain بملء الشاشة مع VSync لثبات 120 FPS
    ...
    // 6. صورة العمق (Depth Buffer) ثلاثية الأبعاد
    ...
    // 8. إنشاء الـ Framebuffers لكل فريم
    ...
    // 9. إنشاء وتثبيت مؤشر الـ UBO الدائم (Persistent Mapping لأقصى أداء)
    ...
    // تحرير كائنات الشيدر فوراً بعد بناء البايبلاين لتوفير الذاكرة
    vkDestroyShaderModule(device, cVsMod, nullptr);
    vkDestroyShaderModule(device, cFsMod, nullptr);

    // (ب) خط أنابيب خطوط أرضية بلندر والجزمو
    ...
    // (ج) خط أنابيب الجزمو ثلاثي الأبعاد (يظهر دائماً فوق كل شيء بدون حجب)
    VkPipelineDepthStencilStateCreateInfo gPdssi = pdssi;
    gPdssi.depthTestEnable = VK_FALSE; // يظهر دائماً في الواجهة
    ...
    // 11. هندسة المكعب الافتراضي لبلندر (24 رأس لتحديد حدة الحواف + 36 فهرس)
    ...
    // 12. شبكة أرضية بلندر الرسمية (المحور الأحمر X والأخضر Z)
    ...
    // 13. تهيئة وتخزين الجزمو ثلاثي الأبعاد
    gizmo.init();
    ...
    LOGI("نجح إقلاع محرك Vulkan بأقصى أداء وبدون تسريب ذاكرة!");
    return true;
}

// دالة التحرير الشاملة - تضمن استرجاع كل بايت تم حجزه في رام الهاتف وكرت الشاشة
void VulkanRenderer::cleanup() {
    ...
    LOGI("تم تحرير كافة موارد الـ Vulkan والذاكرة بالكامل بنجاح 100%!");
}
...
void VulkanRenderer::renderFrame() {
    ...
    // تحديث مصفوفات الكاميرا والضوء عبر الذاكرة المباشرة (Persistent Mapping)
    if (g_uboMapped) {
    ...
    // لون خلفية استوديو بلندر الرمادية الأصلية
    VkClearValue clearValues[2];
    clearValues[0].color = {{0.18f, 0.19f, 0.22f, 1.0f}};
    ...
    // 2. رسم المكعب بشيدر استوديو بلندر الرباعي
    ...
    // 3. رسم الجزمو ثلاثي الأبعاد في الواجهة
